...
contract Ownable {
    ...
    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event OwnershipTransferred(address currentOwner);
}
...
contract Pausable is Ownable {
    ...
    //  2) create a public setter using the inherited onlyOwner modifier
    function setPausable(bool mode) public onlyOwner{
        require(_paused != mode, 'Nothing to change');
        _paused = mode;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(_tokenOwner[tokenId] != to, 'given address should not be the owner of given tokenId');

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == getOwner() || isApprovedForAll(_tokenOwner[tokenId], to), 'caller needs to be the contract owner or isApprovedForAll() is true');
...
//        _symbol=symbol;
//    }
//    function setBaseTokenURI(string memory baseTokenId) external{
//        _baseTokenURI = baseTokenId;
//    }
...
//  2) create a public mint() function that does the following:
...
contract ERC721Mintable is ERC721Metadata {
    ...
    function mint(address to, uint256 tokenId ) public onlyOwner returns(bool){
        require(msg.sender == getOwner(), 'can only be executed by the contract owner');
        super._mint(to, tokenId);
        setTokenURI(tokenId);
        return true;
    }
